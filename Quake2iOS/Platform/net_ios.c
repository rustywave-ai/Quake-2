/*
 * net_ios.c
 * iOS network layer for Quake 2
 * Based on linux/net_udp.c - uses POSIX BSD sockets which work on iOS
 */
...
void NET_SendPacket(netsrc_t sock, int length, void *data, netadr_t to)
{
    ...
    sendto(net_socket, data, length, 0, (struct sockaddr *)&addr, sizeof(addr));
}
...
{
    /* On iOS (single-player only), we don't need to sleep on network */
    (void)msec;
}
